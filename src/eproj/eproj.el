...
;; ‘getCurrentDirectory’ in cabal project shadown ‘System.Directory.getCurrentDirectory’
;; which is wrong.
;;
...
(defun eproj-with-all-project-files-for-navigation (proj func)
  "Prepare all files related to project PROJ that user might want to quickly
...
(defun eproj--navigation-globs (proj)
  ...
  (let ((globs
         (append (eproj-project/extra-navigation-globs proj)
                 '("*.org" "*.md" "*.markdown" "*.rst" "*.json" "*.sh" "*.mk" "*.txt" "*.yaml" "*.xml" "*.nix" "makefile*" "Makefile*" "*.inc" "*.spec" "README" "ChangeLog*" "Changelog*" "*.toml" "*.patch" "*.diff" "*.htm" "*.html" "*.css"))))
