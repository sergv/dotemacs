...
(defun haskell-smart-operators-hash ()
  ...
  (let* ((pt-before (save-excursion
    ...
    (cl-destructuring-bind
      ...
      (cl-destructuring-bind
          (_start _end _is-before? _is-after? is-surrounded-for-c2hs?)
          (smart-operators--point-surrounded-by--impl pt-before pt-after before after ?\{ ?\})
        (cl-destructuring-bind
            (_start _end _is-before? _is-after? is-surrounded-for-unboxed-tuple?)
            (smart-operators--point-surrounded-by--impl pt-before pt-after before after ?\( ?\))
          (cond (is-surrounded-for-pragma?
                ...
                (is-surrounded-for-c2hs?
                 ...
                 (insert "##")
                 (forward-char -1))
                ((and is-surrounded-for-unboxed-tuple?
                      (haskell-ext-tracking-have-unboxed-tuples?))
                 (delete-region pt-before pt-after)
                 (insert "#  #")
                 (forward-char -2))
