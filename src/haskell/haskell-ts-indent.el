...
(defun haskell-ts-indent--prev-adaptive-prefix (_ parent bol)
  (save-match-data
              ...
              (let ((left-child (haskell-ts-getters--infix-left-operand curr))
                ...
                (cond
                  ...
                  ((or (equal prev1 right-child)
                   ...
                   (when (treesit-utils-is-standalone-node? op-child)
                     (if (treesit-utils-is-standalone-node? right-child)
                  ...
                  (let ((in (haskell-ts-indent--get-let-node-in curr)))
                    (cond
                      ((and prev1
                            ...
                            (treesit-utils-is-standalone-node? in))
...
(defun haskell-ts-indent--standalone-vertical-infix-operator-parent (node parent _bol)
  (save-excursion
    (let ((curr parent)
          (prev1 node)
          (tmp nil))
...
(defun haskell-ts-haddock--haddock-arg-doc-anchor (node parent _bol)
...
(defun haskell-ts-haddock--haddock-result-doc-anchor (node _parent _bol)
...
(defun haskell-ts-indent--function-arrow-anchor (node _parent _bol)
  (when-let* ((candidate (haskell-ts-indent--get-topmost-function-node node)))
    (let ((candidate-type (treesit-node-type candidate)))
      (cond
        ...
        ((string= candidate-type "gadt_constructor")
         (haskell-ts-indent--get-gadt-constructor-double-colon candidate))
        (t
         candidate)))))
...
(defun haskell-ts-indent--context-anchor (anchor)
  (pcase (treesit-node-type anchor)
    ...
    ("gadt_constructor"
     (haskell-ts-indent--get-gadt-constructor-double-colon anchor))
    ("parens"
     anchor)
    (_
     (error "Unhandled anchor: %s" anchor))))

(defun haskell-ts-indent--forall-anchor (parent)
  (let ((double-colon (pcase (treesit-node-type parent)
                        ("signature"
                         (haskell-ts-indent--get-signature-double-colon parent))
                        ("gadt_constructor"
    ...
    (let ((rules
           `(((and (node-is "haddock")
             ...
             (,(lambda (node _ _)
              ...
              haskell-indent-offset)
             ((n-p-gp '("," "}" "ERROR") '("record" "fields") nil)
             ...
             ((and (parent-is "bind")
                   (field-is "expression"))
              ,(lambda (_ parent _)
                 (let ((arrow (treesit-node-child-by-field-name parent "arrow")))
                   (if (and arrow
                            (treesit-utils-is-standalone-node? arrow))
                       arrow
                     parent)))
              haskell-indent-offset)
             ...
             ((parent-is "do")
              ,(lambda (node parent bol)
                 (when-let* ((matched-anchor
                              (haskell-ts-indent--standalone-non-infix-parent-or-let-bind-or-field-update-no-list-or-tuple-parent node parent bol)))
             ...
             ((n-p-gp "forall" '("signature" "gadt_constructor") nil)
              ,(lambda (_ parent _)
                 (haskell-ts-indent--forall-anchor parent))
              ,(lambda (_ _ _)
                 (lambda (matched-anchor)
                   (if (string= "::" (treesit-node-type matched-anchor))
             ...
             ((n-p-gp "::" '("signature" "gadt_constructor") nil)
              parent
              haskell-indent-offset)

             ;; NB "prefix" is for GADTs.
             ((and (n-p-gp nil "function" "prefix")
                   (field-is "parameter"))
             ...
             ((or
               ...
               ;; function
               (and (n-p-gp nil "function" '("forall" "parens" "signature" "context" "prefix"))
                    (field-is "parameter")))
