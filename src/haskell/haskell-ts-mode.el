          ...
          (if-let* ((variable (treesit-node-child-by-field-name name "variable")))
            ...
            (when-let* ((type (treesit-node-child-by-field-name name "type")))
              (let ((str (treesit-node-text-no-properties-unsafe type)))
                (puthash str
                         ;; NB whole import_name is added here to be
...
will become nested lists."
  ...
  (-map (lambda (x)
          (let ((typ2 (treesit-node-type x)))
            (pcase typ2
              ((or "apply" "infix")
               (haskell-ts--extract-single-constraint-name-with-children x typ2 str))
              (_
               (treesit-node-text-no-properties-unsafe x str)))))
        (pcase typ
          ("name"  (list node))
