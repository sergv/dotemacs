...
(defcustom cldoc-idle-delay 0.5
...
the mode, respectively."
  ...
  ;; (add-hook 'post-command-hook 'cldoc-pre-command-refresh-echo-area)
  (cond (cldoc-use-idle-timer-p
         (add-hook 'post-command-hook 'cldoc-schedule-timer)
         ;; I commented this out to make it less annoying - even after
         ;; leaving sexp the message keeps appearing when this line is in action
...
(defun cldoc-display-message-p ()
  (cldoc-display-message-no-interference-p)
