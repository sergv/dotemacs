...
(defcustom pabbrev-idle-timer-verbose nil
...
(defcustom pabbrev-minimal-expansion-p t
...
(defun pabbrev-sort-alist (alist cons)
  ...
  (sort alist
        ;;'pabbrev-comparator-function))
        (lambda (a b)
          (> (cdr a) (cdr b)))))

(defun pabbrev-comparator-function (a b)
  (> (cdr a) (cdr b)))
...
;;\new
(defvar pabbrev-insert-suggestion-format "%s"
  "Format of displaying suggested completion")

(defvar pabbrev-insert-suggestion-left-offset 0
  "Offset from the left to leave space unhighlighted in inserted suggestion")
(defvar pabbrev-insert-suggestion-right-offset 0
  "Offset from the right to leave space unhighlighted in inserted suggestion")
  ...
  (let* ((suggestion
    ...
    (save-excursion
     (if (< 0 len)
       ...
       (pabbrev-save-buffer-modified-p
        (insert (format pabbrev-insert-suggestion-format expansion))
        ...
        (setq
         ...
         pabbrev-marker
         (cons end (point)))
        (let ((point-offset (- (point) pabbrev-insert-suggestion-left-offset)))
          (pabbrev-set-overlay
           (- (- (point) len) pabbrev-insert-suggestion-right-offset) point-offset
           (length suggestions))))))))
...
With prefix argument, bring up the menu of all full expansions."
  (if (= uarg 4)
    (if (> (length pabbrev-expansion-suggestions) 1)
      ...
      (pabbrev-suggestions-show-menu pabbrev-last-expansion-suggestions)
...
(defun pabbrev-show-menu ()
  "Show menu with possible completions of previous prefix"
  (interactive)
  (let* ((thing (pabbrev-thing-at-point))
         (suggestions (pabbrev-fetch-all-suggestions-for-prefix
                       thing)))
    (cond
      ((> (length suggestions) 1)
       (pabbrev-suggestions-show-menu suggestions))
      ((= 1 (length suggestions))
       (pabbrev-suggestions-insert (caar suggestions)))
      (t
...
(defun pabbrev-suggestions-show-menu (suggestion-list)
  "Show suggestions menu."
  ;;(message   "Show suggestions menu.")
  (pabbrev-suggestions-menu suggestion-list ""))


(defvar pabbrev-menu-keymap
  popup-menu-keymap)

(defun pabbrev-suggestions-menu (suggestions prefix)
  "Form menu of suggestion candidates"
  ...
  (let* ((suggestion-list (map #'car
    ...
    (setq choosed-suggestion (popup-menu* suggestion-list
                                          ...
                                          :height 20
                                          :scroll-bar t
                                          :keymap pabbrev-menu-keymap
                                          :symbol t))
    (when choosed-suggestion
      (pabbrev-suggestions-insert choosed-suggestion))))
...
(defun pabbrev-suggestions-sort (suggestions)
  "Sort suggestions by their frequency."
  (delq nil
        (sort suggestions
              (lambda (a b)
                (< (cdr a) (cdr b))))))
...
;; we now have menu, buffer is unnecessary
...
(defun pabbrev-thing-at-point()
  "Get thing at point."
  (let ((b (pabbrev-bounds-of-thing-at-point)))
    (buffer-substring-no-properties (car b)
                                    (cdr b))))
...
(defun pabbrev-scavenge-buffer()
  ...
  (save-excursion
   ...
   (message "pabbrev scavenging buffer")
   (while (pabbrev-forward-thing)
