;; popup.el --- Visual popup interface --- -*- lexical-binding: t; -*-
...
(defvar popup-menu-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "s")        'popup-open)
    (define-key map "\C-b"           'popup-close)
    (define-key map [left]           'popup-close)
    (define-key map (kbd "h")        'popup-close)

    (define-key map [down]           'popup-next)
    (define-key map (kbd "t")        'popup-next)
    (define-key map [up]             'popup-previous)
    (define-key map (kbd "n")        'popup-previous)

    (define-key map (kbd "\C-?")     'popup-help)
    (define-key map (kbd "\C-h")     'popup-help)

    (define-key map (kbd "/")        'popup-isearch)
    (define-key map (kbd "/")        'popup-isearch)
    (define-key map (kbd "<escape>") 'popup-exit)
    map))
