...
Used by `py-electric-colon', which will not indent than. "
  (let (erg)
    (save-excursion
      (beginning-of-line)
      (setq erg (or (looking-at-pure? py-class-re)
                    (looking-at-pure? py-def-re))))
    erg))
          ...
          (cond ((eq indent cui)
                ...
                (t (beginning-of-line)
                   ...
                   (if (<= (line-beginning-position) (+ (point) (- col cui)))
                     ...
                     (beginning-of-line))))
        (insert-tab))))
  (when (and (interactive-p) py-report-level-p)
...
initial line; and comment lines beginning in column 1 are ignored."
  ...
  (save-excursion
    ...
    (let ((py-indent-offset (prefix-numeric-value
          ...
          (indent-base (if (looking-at-pure? "[ \t\n]")
      ...
      (while (< (point) end)
        ...
        (cond
         ((or (eq (following-char) ?#)  ; comment in column 1
              (looking-at-pure? "[ \t]*$"))   ; entirely blank
    ...
    (save-restriction
      ...
      (let ((pps
        ...
        (cond
         ...
         ((save-excursion (beginning-of-line) (looking-at-pure? "[ \t]*#"))
          (py-fill-comment justify))
         ;; inside a string
         ((nth 3 pps)
            ...
            (cond
              ...
              ((and (bobp)
               ...
               (+ (if py-smart-indentation
                    (py-guess-indent-offset nil orig)
                    py-indent-offset)
                  (current-indentation)))
              ((and (bobp)
                    (not (py-statement-opens-block-p)))
               (current-indentation))
              ;; inside a string
              ((and (nth 3 pps)
                    ;; character address of start of string
                    (nth 8 pps))
              ...
              ((nth 8 pps)
               (if (eq origline (py-count-lines))
                 ...
                 (progn
                   (goto-char (nth 8 pps))
                   (if (and line
                            (or py-indent-honors-inline-comment
                                (looking-back "^[ \t]*")))
                     (current-column)
                     (forward-char -1)
                     (py-compute-indentation orig
                       ...
                       (cond
                         ((< 0 (- origline this-line))
                          (if (< 1 (- origline this-line))
                            ...
                            (cond ((looking-at-pure? "\\s([ \t]*$")
                                   (if
                                       (progn
                                         (save-excursion
                                          (back-to-indentation)
                                          (looking-at-pure? py-block-or-clause-re)))
                                     (progn
                                       (back-to-indentation)
              ...
              ((py-preceding-line-backslashed-p)
               (progn
                 ...
                 (if (< 1 (- origline this-line))
                   (py-fetch-previous-indent orig)
                   (if (looking-at-pure? "from +\\([^ \t\n]+\\) +import")
                     5
                     (+ (current-indentation) py-continuation-offset)))))
              ((looking-at-pure? py-no-outdent-re)
               (if (eq (py-count-lines) origline)
                 (progn
                   ...
                   (py-compute-indentation orig origline closing line inside repeat))
                 (current-indentation)))
              ((and (looking-at-pure? py-block-closing-keywords-re)
                    (eq (py-count-lines) origline))
               (py-beginning-of-block-or-clause)
               (+  (if py-smart-indentation
                     (py-guess-indent-offset nil orig)
                     py-indent-offset)
                   (current-indentation)))
              ((looking-at-pure? py-block-closing-keywords-re)
               (py-beginning-of-block-or-clause nil (current-indentation))
               (current-indentation))
              ((and (looking-at-pure? py-elif-re) (eq (py-count-lines) origline))
               (py-line-backward-maybe)
               (car (py-clause-lookup-keyword py-elif-re -1)))
              ((and (looking-at-pure? py-clause-re)
                    (eq origline (py-count-lines)))
               (cond ((looking-at-pure? py-finally-re)
                      (car (py-clause-lookup-keyword py-finally-re -1)))
                     ((looking-at-pure? py-except-re)
                      (car (py-clause-lookup-keyword py-except-re -1)))
                     ((looking-at-pure? py-else-re)
                      (car (py-clause-lookup-keyword py-else-re -1 (current-indentation))))
                     ((looking-at-pure? py-elif-re)
                      (car (py-clause-lookup-keyword py-elif-re -1)))))
              ((looking-at-pure? py-block-or-clause-re)
               (cond ((eq origline (py-count-lines))
                      (py-line-backward-maybe)
                      (py-compute-indentation orig origline closing line inside t))
                     (t (+ (if py-smart-indentation
                             (py-guess-indent-offset nil orig)
                             py-indent-offset)
                           (current-indentation)))))
              ((looking-at-pure? py-block-closing-keywords-re)
              ...
              ((and (< (py-count-lines) origline)
                    (looking-at-pure? py-assignment-re))
               (current-indentation))
              ((looking-at-pure? py-assignment-re)
               (py-beginning-of-statement)
               (py-compute-indentation orig origline closing line inside repeat))
              ((and (eq origline (py-count-lines))
                    (save-excursion
                     (and (setq erg
                                (py-go-to-keyword py-block-or-clause-re -1))
                          (ignore-errors
                           (< orig (py-end-of-block-or-clause))))))
               (+ (car erg)
                  (if py-smart-indentation
                    (py-guess-indent-offset nil orig)
                    py-indent-offset)))
...
(defun py-in-comment-p ()
  ...
  (save-restriction
    ...
    (let* ((pps
      ...
      (unless erg
        (when (looking-at-pure? (concat "^[ \t]*" comment-start-skip))
          (setq erg (point))))
      erg)))
...
(defun py-outdent-p ()
  ...
  (save-excursion
    (and (progn (back-to-indentation)
                (looking-at-pure? py-clause-re))
         ;; short circuit infloop on illegal construct
         (not (bobp))
         (progn (forward-line -1)
                (py-beginning-of-statement)
                (back-to-indentation)
                (when (looking-at-pure? py-blank-or-comment-re)
                  (backward-to-indentation 1))
                (not (looking-at-pure? py-no-outdent-re))))))
        ...
        (setq erg
              (cond
               ...
               ((nth 8 pps)
                ...
                (py-beginning-of-expression orig origline))
               ((and (looking-at-pure? "[ \t]*#") (looking-back "^[ \t]*"))
                (forward-line -1)
                (unless (bobp)
                  (end-of-line)
                  (py-beginning-of-expression orig origline)))
               ...
               ((and (eq (point) orig) (not (bobp)) (looking-back py-expression-looking-regexp))
                (skip-chars-backward py-expression-skip-regexp)
                (py-beginning-of-expression orig origline))
               ((looking-at-pure? py-expression-looking-regexp)
                (point))
               (t (unless (and (looking-at-pure? "[ \t]*#") (looking-back "^[ \t]*"))(point)))))
        (when (interactive-p) (message "%s" erg))
        erg))))
      ...
      (let*
        ...
        (cond
         ...
         ((nth 4 pps)
          ...
          (py-end-of-expression orig origline done))
         ((and (looking-at-pure? "[ \t]*#")
               (looking-back "^[ \t]*")(not done))
          (while (and (looking-at-pure? "[ \t]*#")
                      (forward-line 1)
                      (not (eobp))
                      (beginning-of-line)))
          (end-of-line)
         ...
         ((nth 1 pps)
          ...
          (let ((parse-sexp-ignore-comments t))
            (forward-list)
            (py-end-of-expression orig origline done)))
         ((and (not done)(looking-at-pure? py-not-expression-regexp)(not (eobp)))
          (skip-chars-forward py-not-expression-regexp)
          (py-end-of-expression orig origline done))
         ((and (not done)(looking-at-pure? py-expression-skip-regexp)(not (eobp)))
          (skip-chars-forward py-not-expression-regexp)
          (forward-char -1)
          (py-end-of-expression orig origline done))
         ((and (looking-at-pure? py-expression-looking-regexp)(not (eobp)))
          (forward-char 1)
        ...
        (setq erg
              (cond
               ...
               ((and (eq (point) orig) (not (bobp)) (looking-back py-minor-expression-looking-regexp))
                ...
                (py-beginning-of-partial-expression orig origline))
               ((looking-at-pure? py-minor-expression-looking-regexp)
                (point))
               (t (unless (and (looking-at-pure? "[ \t]*#") (looking-back "^[ \t]*"))(point)))))
        (when (interactive-p) (message "%s" erg))
        erg))))
      ...
      (let*
        ...
        (cond
         ...
         ((nth 4 pps)
          (forward-line 1)
          (py-end-of-partial-expression orig origline done))
         ((and (looking-at-pure? "[ \t]*#")(looking-back "^[ \t]*")(not done))
          (while (and (looking-at-pure? "[ \t]*#") (forward-line 1)(not (eobp))
                      (beginning-of-line)))
          (end-of-line)
          ;;          (setq done t)
          (skip-chars-backward " \t\r\n\f")
         ...
         ((and (nth 1 pps) (<= orig (nth 1 pps)))
          ...
          (let ((parse-sexp-ignore-comments t))
            ...
            (setq done t)
            (py-end-of-partial-expression orig origline done)))
         ((and (looking-at-pure? "\\.")(< orig (point)))
          (point))
         ((and (not done)
               (looking-at-pure? "\\.\\|=\\|:\\|+\\|-\\|*\\|/\\|//\\|&\\|%\\||\\|\^\\|>>\\|<<\\|<\\|<=\\|>\\|>=\\|==\\|!="))
          (goto-char (match-end 0))
         ...
         ((and (not done)
          ...
          (py-end-of-partial-expression orig origline done))
         ((and (not done)
               (looking-at-pure? py-not-minor-expression-regexp)
               (not (eobp)))
      ...
      (let ((orig (or orig (point)))
        ...
        (cond
         ...
         ((and (eq (point) orig)(or (bolp) (<= (current-column)(current-indentation))))
          ...
          (skip-chars-backward " \t")
          (py-beginning-of-statement orig origline done)))
        (unless (and (looking-at-pure? "[ \t]*#") (looking-back "^[ \t]*"))
          (when (< (point) orig)(setq erg (point))))
        (when (interactive-p) (message "%s" erg))
        erg))))
          ...
          (cond
           ...
           ((and (looking-at-pure? "[ \t]*#")(looking-back "^[ \t]*"))
            (while (and (looking-at-pure? "[ \t]*#") (forward-line 1)(not (eobp))
              ...
              (setq done t))
            (end-of-line)
            (when (and done (looking-at-pure? "[ \t]*$") (not (looking-back "^[ \t]*")))
           ...
           ((and (bolp) (not (empty-line-p)))
            ...
            (setq done t)
            (py-end-of-statement orig origline done))
           ((looking-at-pure? "\\.\\([A-Za-z_][A-Za-z_0-9]*\\)")
            (forward-char 1)
...
Returns position if succesful "
  (interactive)
  (back-to-indentation)
  (while (and (not (looking-at-pure? "@\\w+"))(not (empty-line-p))(not (bobp))(forward-line -1))
    (back-to-indentation))
  (let ((erg (when (looking-at-pure? "@\\w+")(match-beginning 0))))
    (when (interactive-p) (message "%s" erg))
    erg))
...
(defun py-clause-lookup-keyword (regexp arg &optional indent)
  ...
  (let ((orig (point))
        ...
        (complement-re
          (cond ((string-match-p "finally\\|except" regexp)
                 py-try-re)
                ((string-match-p "elif" regexp)
                 py-if-re)
                ((string-match-p "else" regexp)
                 py-minor-block-re)))
        (first t)
        erg done)
    (while (and (not (eval stop))
      ...
      (when (< (current-indentation) maxindent)
        (unless (looking-at-pure? py-block-or-clause-re)
          (setq maxindent (current-indentation)))
        ;; (message "%s %s" count indent)
        ;; nesting
        (cond
          ((and (looking-at-pure? "\\_<finally\\>[: \n\t]")(save-match-data (string-match regexp "finally")))
           ...
           (while
               (and
                ...
                (funcall function)
                (setq done t)
                (not (and (eq indent (current-indentation))
                          (looking-at-pure? "try"))))))
          ((and (looking-at-pure? "\\_<expcept\\>[: \n\t]")
                (string-match-p "else" regexp))
           (setq indent (current-indentation))
           (setq count (1+ count))
           (while
               (and
                ...
                (setq done t)
                (not (and (eq indent (current-indentation))
                          (looking-at-pure? "try\\|if"))))))
          ((and (looking-at-pure? "\\_<else\\>[: \n\t]")
                (string-match-p "else" regexp))
           (setq indent (current-indentation))
           (setq count (1+ count))
           (while
          ...
          ((and (looking-at-pure? "\\_<elif\\>[ \n\t]")
           ...
           (while
               (and
                ...
                (not (and (eq indent (current-indentation))
                          (looking-at-pure? "if"))))))
          ((and (looking-at-pure? complement-re)(< (current-indentation) maxindent))
           (setq count (1- count)))
          (t (when (and (string-match-pure? "except" regexp)
                        (looking-at-pure? py-block-re))
               (setq count (1- count)))))))
    (when erg
      (if (looking-at-pure? py-def-or-class-re)
...
(defun py-go-to-keyword (regexp arg &optional maxindent)
  ...
  (let ((orig (point))
    ...
    (while (and (or (not done)(eq origline (py-count-lines)))
                (not (eval stop)))
      (funcall function)
      (when (and (looking-at-pure? regexp)(if maxindent
...
Returns indentation if clause found, nil otherwise. "
  ...
  (let* ((orig (point))
    ...
    (if (eobp)
        (setq erg nil)
      (while (and (setq erg (py-down-statement))(not (looking-at-pure? py-clause-re)))))
    (when (interactive-p) (message "%s" erg))
    erg))
...
Returns indentation if block-or-clause found, nil otherwise. "
  ...
  (let* ((orig (point))
    ...
    (if (eobp)
        (setq erg nil)
      (while (and (setq erg (py-down-statement))(not (looking-at-pure? py-block-or-clause-re)))))
    (when (interactive-p) (message "%s" erg))
    erg))
...
Returns indentation if def found, nil otherwise. "
  ...
  (let* ((orig (point))
    ...
    (if (eobp)
        (setq erg nil)
      (while (and (setq erg (py-down-statement))(not (looking-at-pure? py-def-re)))))
    (when (interactive-p) (message "%s" erg))
    erg))
...
Returns indentation if class found, nil otherwise. "
  ...
  (let* ((orig (point))
    ...
    (if (eobp)
        (setq erg nil)
      (while (and (setq erg (py-down-statement))(not (looking-at-pure? py-class-re)))))
    (when (interactive-p) (message "%s" erg))
    erg))
...
Returns indentation if def-or-class found, nil otherwise. "
  ...
  (let* ((orig (point))
    ...
    (if (eobp)
        (setq erg nil)
      (while (and (setq erg (py-down-statement))(not (looking-at-pure? py-def-or-class-re)))))
    (when (interactive-p) (message "%s" erg))
    erg))
...
With universal arg \C-u insert a `%'. "
  ...
  (let ((parse-sexp-ignore-comments t))
    (if arg
      ...
      (cond
       ((and (not match-paren-no-use-syntax-pps) (looking-at-pure? "\\s("))
        (forward-list 1)
        (backward-char 1))
       ((and (not match-paren-no-use-syntax-pps)(looking-at-pure? "\\s)"))
...
This function is appropriate for `comint-output-filter-functions'."
  ...
  (when (and (string-match-p py-shell-input-prompt-1-regexp string)
             py-file-queue)
...
(defun py-execute-base (start end &optional async shell dedicated)
  ...
  (let* ((regbuf (current-buffer))
         (py-execute-directory (or (ignore-errors
                                    (file-name-directory (buffer-file-name)))
                                   (getenv "HOME")))
         (name-raw (or shell (py-choose-shell)))
         (name (py-process-name name-raw))
         (strg (py-clean-up-buffer-contents
                name
                (buffer-substring-no-properties start end)))
         (file (concat py-temp-directory "/tmp.py"))
    ...
    ;; (set-buffer regbuf)
    (unless (file-directory-p py-temp-directory)
      (mkdir py-temp-directory t))
    (py-execute-intern strg procbuf proc file filebuf name py-execute-directory)))

(defun py-execute-intern (strg
                          &optional
                          procbuf
                          proc
                          file
  ...
  (let ((pec (if (string-match "Python3" name)
               (format "exec(compile(open('%s').read(), '%s',
                 'exec')) # PYTHON-MODE\n" file file)
               (format "execfile(r'%s') # PYTHON-MODE\n" file)))
        shell)
    (set-buffer filebuf)
...
the next line."
  ...
  (if ipython-autoindent
      (let ((line (buffer-substring (point-at-bol) (point)))
        ...
        (save-excursion
          (comint-bol t)
          (if (looking-at-pure? py-shell-input-prompt-1-regexp)
              (setq after-prompt1 t)
            (setq after-prompt2 (looking-at-pure? py-shell-input-prompt-2-regexp)))
...
(defun py-if-needed-insert-shell (&optional name)
  (let ((erg (if name (downcase name)
    ...
    (unless (looking-at-pure? py-shebang-regexp)
  ...
  (let ((curbuf (current-buffer))
    ...
    (unwind-protect
         (save-excursion
          ...
          (message "PY-EXECUTE-FILE: PROC: %s, (PROCESS-FILTER PROC): %s"
                   (pp-to-string proc)
                   (pp-to-string (process-filter proc)))
    ...
    (unless (eobp)
      (if (and (py-in-comment-p)(not py-indent-comments))
        ...
        (unless (eq 4 (prefix-numeric-value arg))
          (if (eobp) (newline)
            (progn (forward-line 1))
            (when (and py-kill-empty-line (empty-line-p) (not (looking-at-pure? "[ \t]*\n[[:alpha:]]")) (not (eobp)))
...
(defun py-looking-at-keywords-p ()
  ...
  (let* ((kwds1 (car (nth 1 (eval (eval (quote (car font-lock-defaults)))))))
	 ...
	 (res
	  (or
           (looking-at-pure? kwds1)
           (looking-at-pure? kwds3))))
