    ...
    (while cont
      ...
      (let ((token-str nil)
        ...
        (cond
          ...
          ((memq (setf token-char (haskell-font-lock--match-char 0))
                 '(?\) ?\] ?\}))
           ;; (member (setf token-str (match-string-no-properties 0))
           ;;         '(")" "]" "}"))
          ...
          ((and (member token-char
                        '(?\, ?\; ?\|))
                (not (member (setf token-str (match-string-no-properties 0))
                             ignore)))
           (if (eq 0 open-parens)
               (setq cont nil)
             ...
             (goto-char end))
           (setq last-token-was-newline nil))
          ((and (or (member (or token-str
                                (setf token-str (match-string-no-properties 0)))
                            '("<-" "=" "←"))
                    (member token-str haskell-font-lock-keywords))
                (not (member token-str ignore)))
           (setq cont nil)
           (setq last-token-was-newline nil))
          ((member token-char
                   '(?\( ?\[ ?\{))
           (if last-token-was-newline
             ...
             (goto-char (setq end (match-end 0)))
             (setq last-token-was-newline nil)))
          ((memq token '(qsymid char string number template-haskell-quote template-haskell-quasi-quote))
           (setq last-token-was-operator (memq (haskell-lexeme-classify-by-first-char (char-after (match-beginning 1)))
                                               '(varsym consym)))
           (if (and (not last-token-was-operator) last-token-was-newline)
             ...
             (goto-char (setq end (match-end 0))))
           (setq last-token-was-newline nil))
          ((memq token '(comment nested-comment literate-comment))
           (goto-char (setq end (match-end 0))))
          (t
           (goto-char (setq end (match-end 0)))
           (setq last-token-was-newline nil)))))
