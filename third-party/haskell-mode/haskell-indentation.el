   ...
   ((and (memq last-command
    ...
    (let ((mark-even-if-inactive t))
      (haskell-indentation-indent-rigidly
       ...
       1))
    t)
   ((and (or (memq last-command '(indent-for-tab-command haskell-indentation-indent-line))
             (and (eq last-command 'yas-expand-or-fallback)
                  (eq yas-expand-fallback #'haskell-indentation-indent-line)))
...
(defvar haskell-indentation-in-guard?)
(defvar haskell-indentation-in-do?)
(defvar haskell-indentation-in-let?)
...
(defun haskell-indentation-parse-to-indentations ()
  ...
  (save-excursion
    ...
    (let ((indentation-point (point))
          ...
          haskell-indentation-in-guard?
          haskell-indentation-in-do?
          haskell-indentation-in-let?)
  ...
  `(("::" .
     (lambda ()
       (haskell-indentation-with-starter
        (lambda ()
          (haskell-indentation-separated #'haskell-indentation-type
                                         '("->" "=>"))))))
    ("("  .
    ...
    ("proc"    .
     ,(apply-partially #'haskell-indentation-phrase
                       '(haskell-indentation-expression
                         ...
                         haskell-indentation-expression)))
    ("where"   .
     ,#'haskell-indentation-expression-where)
    ("::"      .        haskell-indentation-scoped-type)
    ("="       .        haskell-indentation-after-equals)
    ("<-"      .
     ,(apply-partially #'haskell-indentation-statement-right
                       #'haskell-indentation-expression))
    ("("       .
     ,(lambda ()
        (haskell-indentation-list #'haskell-indentation-expression
                                  ")"
                                  '("," "->"))))
...
(defun haskell-indentation-after-equals ()
  (haskell-indentation-statement-right #'haskell-indentation-expression))

(defun haskell-indentation-expression-where ()
  (haskell-indentation-with-starter #'haskell-indentation-declaration-layout
                                    nil
                                    t))
...
(defun haskell-indentation-fundep1 ()
  ...
  (let ((current-indent (current-column-fixed)))
...
(defun haskell-indentation-data ()
  ...
  (cond ((eq current-token 'end-tokens)
         (when (member following-token '("=" "where"))
           ...
           (throw 'parse-end nil)))
        ((equal current-token "=")
         (let ((starter-indent-inside (current-column-fixed)))
           (haskell-indentation-with-starter
            (lambda ()
           ...
           (cond
            ((equal current-token 'end-tokens)
             (when (equal following-token "deriving")
...
(defun haskell-indentation-deriving ()
  ...
  (haskell-indentation-with-starter
   (lambda ()
     (when (equal "instance" current-token)
       (haskell-indentation-read-next-token))
  ...
  (haskell-indentation-with-starter
   (lambda ()
     (let ((implicit-layout-active nil))
       (haskell-indentation-separated parser sep stmt-sep)))
   end))
...
Skip the keyword or parenthesis." ; FIXME: better description needed
  (let ((starter-column (current-column-fixed))
        (current-indent current-indent)
        (left-indent
         (if (= (current-column-fixed) (haskell-indentation-current-indentation))
             (current-column-fixed)
           left-indent)))
    (haskell-indentation-read-next-token)
    (when (eq current-token 'end-tokens)
...
(defun haskell-indentation-case-alternative ()
  "" ; FIXME
  (setq left-indent (current-column-fixed))
...
(defun haskell-indentation-case ()
  ...
  (cond ((eq current-token 'end-tokens)
        ...
        ((equal current-token "|")
         (haskell-indentation-with-starter
          (apply-partially #'haskell-indentation-separated
                           #'haskell-indentation-case-alternative
                           '("|")
                           nil)
          nil))
        ((equal current-token "->")
...
(defun haskell-indentation-guard ()
  ...
  (let ((haskell-indentation-in-guard? (haskell-indentation-current-indentation)))
    (setq left-indent (current-column-fixed))
    (haskell-indentation-separated (lambda () (haskell-indentation-expression nil '("=")))
                                   '(",")
                                   nil)
    (cond
      ((equal current-token "|"))
      ((equal current-token "=")
       (haskell-indentation-read-next-token)
       (haskell-indentation-expression nil '("where"))
       (let ((haskell-indentation-in-guard? nil))
         (cond
           ((equal current-token "where")
            (haskell-indentation-expression-where))
           ((eq current-token 'end-tokens)
...
(defun haskell-indentation-declaration ()
  ...
  (haskell-indentation-separated (lambda ()
  ...
  (cond
    ...
    ((equal current-token "|")
     (haskell-indentation-with-starter
      (lambda ()
        (haskell-indentation-separated #'haskell-indentation-guard '("|") nil))
      nil))
    ((equal current-token "=")
     (haskell-indentation-after-equals)))
  (when (eq current-token 'end-tokens)
    (cond
      ((and haskell-indentation-in-let?
            (equal following-token "in"))
       (haskell-indentation-push-indentation haskell-indentation-in-let?)
       (throw 'parse-end nil))
      ((and haskell-indentation-in-guard?
            (member following-token '("|" "=" ",")))
       (haskell-indentation-push-indentation haskell-indentation-in-guard?)
       (throw 'parse-end nil))
      ((member following-token '("|" "=" "::"))
       (haskell-indentation-add-indentation current-indent)
       (throw 'parse-end nil)))))
...
(defun haskell-indentation-expression (&optional accepted-tokens stop-tokens)
  ...
  (catch 'return
    (let ((current-indent (current-column-fixed)))
      ...
      (while t
        (cond
          ((member current-token accepted-tokens)
           (haskell-indentation-read-next-token))
          ((member current-token stop-tokens)
           (throw 'return nil))
          ((eq current-token 'end-tokens)
           (cond ((equal following-token "where")
...
(defun haskell-indentation-separated (parser separator &optional stmt-separator guard-override)
    ...
    (let ((separator-indent nil))
      (while t
        ...
        (cond ((member current-token separator)
               (setq separator-indent (current-column-fixed))
               (haskell-indentation-at-separator))

              ((member current-token stmt-separator)
               (setq starter-indent (current-column-fixed)
                     separator-indent (current-column-fixed))
               (haskell-indentation-at-separator))

              ((eq current-token 'end-tokens)
               (unless guard-override
                 (when (or (member following-token separator)
                   ...
                   ;;  , 2            , 20
                   (if separator-indent
                       (haskell-indentation-push-indentation separator-indent)
                     (progn
                       (haskell-indentation-add-indentation starter-indent)
...
l = [  1
  ...
  (let* ((col (current-column-fixed))
         (separator-column
          (and (= col (haskell-indentation-current-indentation))
               col)))
    (haskell-indentation-read-next-token)
    (cond ((eq current-token 'end-tokens)
           (haskell-indentation-add-indentation current-indent)
...
layout starts."
  (let* ((layout-indent (current-column-fixed))
         (current-indent (current-column-fixed))
    ...
    (catch 'return
      (while t
        ...
        (cond ((member current-token '(layout-item ";"))
               (haskell-indentation-read-next-token))
              ((eq current-token 'end-tokens)
               (when (or (and (not (member following-token '("{" operator)))
                              (not (eq previous-token 'operator))
                              (haskell-indentation-expression-token-p following-token))
                         (equal following-token ";")
                         (and (equal layout-indent 0)
                              (assoc following-token haskell-indentation-toplevel-list)
                              (not (equal following-token "where"))))
                 (haskell-indentation-add-layout-indent))
               (throw 'return nil))
              (t (throw 'return nil))))))
...
(defun haskell-indentation-phrase (phrase)
  ...
  (haskell-indentation-with-starter
   (lambda ()
     (haskell-indentation-phrase-rest phrase))
   nil))
  ...
  (while phrase1
    (let ((phrase phrase1))
      ...
      (cond
        ...
        ((and haskell-indentation-in-let?
              (or haskell-indentation-in-guard?
                  haskell-indentation-in-do?))
         (throw 'return t))))))
...
(defun haskell-indentation-add-where-pre-indent ()
  ...
  (when (= layout-indent haskell-indentation-layout-offset)
    (haskell-indentation-push-indentation haskell-indentation-where-pre-offset)))
