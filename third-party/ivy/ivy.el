...
(defun ivy-partial--impl (changedir-for-single-dir?)
  "Complete the minibuffer text as much as possible."
...
(with-eval-after-load 'flx
  (setq ivy--flx-cache (flx-make-string-cache)
        ivy--flx-filenames-cache (flx-make-filename-cache)))
...
(defun ivy--flx-available-p ()
  ...
  (if (boundp 'ivy--flx-available-p)
      ivy--flx-available-p
    (setq ivy--flx-available-p (or (require 'flx nil t)
    ...
    (unless (or (memq this-command '(ivy-resume ivy-partial-or-done))
      ...
      (let ((index (cond
                     ...
                     ((and (not empty)
                           ...
                           (not (and (memq ivy--regex-function '(ivy--regex-fuzzy ivy--regex-fuzzy-filenames))
                                     (ivy--flx-available-p)))
...
(defun ivy--flx-sort-filenames-no-match-on-empty (name cands)
  "Sort according to closeness to string NAME the string list CANDS."
  (if (string= name "")
      nil
    (ivy--flx-sort-impl name cands t)))
...
(defun ivy--highlight-fuzzy-impl (str match-filenames?)
  ...
  (if (and (memq (ivy-alist-setting ivy-re-builders-alist) '(ivy--regex-fuzzy ivy--regex-fuzzy-filenames))
           (ivy--flx-available-p))
      (let ((flx-name (string-remove-prefix "^" ivy-text)))
        (ivy--flx-propertize
         (pcase ivy--flx-sort--backend
           ...
           (`elisp
            (flx-score str flx-name (if match-filenames? ivy--flx-filenames-cache ivy--flx-cache)))
           (invalid
            (error "Invalid ivy--flx-sort--backend: %s" invalid)))
         str))
