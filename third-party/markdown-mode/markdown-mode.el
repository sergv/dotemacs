...
Positions are calculated by `markdown-calc-indents'."
  ...
  (let ((positions (markdown-calc-indents))
        (point-pos (current-column-fixed))
        (_ (back-to-indentation))
        (cur-pos (current-column-fixed-uncached)))
...
`backward-delete-char-untabify' ARG times."
  ...
  (if (use-region-p)
    ...
    (let ((cur-pos (current-column-fixed))
  ...
  (let ((mincol 1000))
    (save-excursion
      ...
      (while (< (point) end)
        (back-to-indentation)
        (unless (looking-at-p "[ \t]*$")
          (setq mincol (min mincol (current-column-fixed-uncached))))
        (forward-line 1)))
    mincol))
...
With optional argument UP, move it up."
  ...
  (unless (markdown-table-at-point-p)
    (user-error "Not at a table"))
  (let* ((col (current-column-fixed)) (pos (point))
         (tonew (if up 0 2)) txt)
...
rows and columns and the column alignment."
  ...
  (let* ((rows (or rows (read-number "Number of Rows: ")))
         ...
         (align (cond ((equal align "l") ":--")
                      ...
                      (t "---")))
         (pos (point))
         (indent (make-string (current-column-fixed) ?\ ))
         (line (concat
                (apply 'concat indent "|"
                       (make-list columns "   |")) "\n"))
