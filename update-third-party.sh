...
if [[ "$#" == 3 ]]; then
    hash="$3"
elif [[ "${repository}" == "magit-repo" || "${repository}" == "with-editor-repo" || "${repository}" == "transient-repo" || "${repository}" == "el-patch-repo" || "${repository}" == "git-modes-repo" || "${repository}" == "ghub-repo" ]]; then
